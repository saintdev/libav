...
static av_cold int psy_3gpp_init(FFPsyContext *ctx) {
    ...
    pctx->chan_bitrate = ctx->avctx->bit_rate / ctx->avctx->channels;
    pctx->frame_bits   = ctx->avctx->bit_rate * AAC_BLOCK_SIZE_LONG / ctx->avctx->sample_rate;
    ...
    ctx->bitres.size   = 6144 * ctx->avctx->channels - pctx->frame_bits;
    ...
    for (j = 0; j < 2; j++) {
        ...
        float avg_chan_bits = pctx->chan_bitrate / ctx->avctx->sample_rate * (j ? 128.0f : 1024.0f);
        ...
        float en_spread_hi  = (j || (pctx->chan_bitrate <= 22.0f)) ? PSY_3GPP_EN_SPREAD_HI_S : PSY_3GPP_EN_SPREAD_HI_L1;
 ...
 */
static void psy_3gpp_analyze(FFPsyContext *ctx, int channel, const float **coeffs,
                             const FFPsyWindowInfo *wi)
{
    ...
    for (ch = 0; ch < group->num_ch; ch++) {
        ...
        for (w = 0; w < wi[ch].num_windows*16; w += 16) {
            for (g = 0; g < num_bands[ch]; g++) {
                AacPsyBand *band = &pch->band[w+g];
                float form_factor = 0.0f;

                band->energy = 0.0f;
                for (i = 0; i < band_sizes[g]; i++) {
                    band->energy += coeffs[ch][start+i] * coeffs[ch][start+i];
                    form_factor  += sqrtf(fabs(coeffs[ch][start+i]));
                }
        ...
        for (w = 0; w < wi[ch].num_windows*16; w += 16) {
            ...
            spread_en[ch][0] = bands[0].energy;
            for (g = 1; g < num_bands[ch]; g++) {
                bands[g].thr       = FFMAX(bands[g].thr,    bands[g-1].thr * psy_coeffs[g].spread_hi[0]);
                spread_en[ch][w+g] = FFMAX(bands[g].energy, spread_en[ch][w+g-1] * psy_coeffs[g].spread_hi[1]);
            }
            for (g = num_bands[ch] - 2; g >= 0; g--) {
                bands[g].thr       = FFMAX(bands[g].thr,       bands[g+1].thr * psy_coeffs[g].spread_low[0]);
                spread_en[ch][w+g] = FFMAX(spread_en[ch][w+g], spread_en[ch][w+g+1] * psy_coeffs[g].spread_low[1]);
            }
            //5.4.2.4 "Threshold in quiet"
            for (g = 0; g < num_bands[ch]; g++) {
                AacPsyBand *band = &bands[g];

                band->thr_quiet = band->thr = FFMAX(band->thr, psy_coeffs[g].ath);
    ...
    for (ch = 0; ch < group->num_ch; ch++) {
        ...
        for (w = 0; w < wi[ch].num_windows*16; w += 16) {
            for (g = 0; g < num_bands[ch]; g++) {
                ...
                if (spread_en[ch][w+g] * avoid_hole_thr > band->energy || psy_coeffs[g].min_snr > 1.0f)
                    band->avoid_holes = PSY_3GPP_AH_NONE;
                else
                    band->avoid_holes = PSY_3GPP_AH_INACTIVE;
            }
        }
        short_window |= wi[ch].num_windows == 8;
//         ctx->ch[ch].entropy = pch->pe;
    }

    /* 5.6.1.3.2 "Calculation of the desired perceptual entropy" */
    desired_bits = calc_bit_demand(pctx, pe, ctx->bitres.bits, ctx->bitres.size, short_window);
...
av_log(NULL, AV_LOG_INFO, "5.6.1.3.2: pe = %f, desired = %f, bits = %f\n", pe, desired_pe, desired_bits);
    if (desired_pe < pe) {
        ...
        for (ch = 0; ch < group->num_ch; ch++) {
            ...
            for (w = 0; w < wi[ch].num_windows*16; w += 16) {
                for (g = 0; g < num_bands[ch]; g++) {
                    ...
                    band->thr = calc_reduced_thr_3gpp(band, psy_coeffs[g].min_snr, reduction);
                    /* recalculate PE */
                    pe += calc_pe_3gpp(band);
                    a  += band->pe_const;
                    active_lines += band->active_lines;
...
av_log(NULL, AV_LOG_INFO, "5.6.1.3.4: pe = %f, desired = %f\n", pe, desired_pe);
...
av_log(NULL, AV_LOG_INFO, "5.6.1.3.5: pe = %f, desired = %f\n", pe, desired_pe);
                ...
                for (w = 0; w < wi[ch].num_windows*16; w += 16) {
                    for (g = 0; g < num_bands[ch]; g++) {
                        ...
                        if (band->active_lines > 0.5f) {
                            ...
                            if (thr > psy_coeffs[g].min_snr * band->energy && band->avoid_holes == PSY_3GPP_AH_INACTIVE)
                                thr = FFMAX(band->thr, psy_coeffs[g].min_snr * band->energy);
                            band->thr = thr;
                        }
                    }
                }
            ...
            while (sfb++ < max_bands) {
                for (ch = 0; ch < group->num_ch; ch++) {
                    ...
                    if (g >= 0) {
                        for (w = 0; w < wi[ch].num_windows*16; w+= 16) {
                            AacPsyBand *band = &pch->band[w+g];
                            if (band->avoid_holes != PSY_3GPP_AH_NONE && psy_coeffs[g].min_snr < PSY_SNR_1DB) {
                                psy_coeffs[g].min_snr = PSY_SNR_1DB;
            ...
            if (pe > desired_pe) {
                ...
                for (w = 0; w < wi[0].num_windows*16; w += 16) {
                    for (g = 0; g < num_bands[0]; g++) {
                        AacPsyBand   *band0 = &pctx->ch[channel + 0].band[w+g];
                        AacPsyBand   *band1 = &pctx->ch[channel + 1].band[w+g];
                        AacPsyCoeffs *psy_coeffs = pctx->psy_coef[wi[0].num_windows == 8];

                        if (group->coupling[w+g]) {
                            if (band1->avoid_holes != PSY_3GPP_AH_NONE &&
                                0.4f * psy_coeffs[g].min_snr * band0->energy > band1->energy) {
                                band1->avoid_holes = PSY_3GPP_AH_NONE;
                                band1->thr = 2.0f * band1->energy;
                                pe -= band1->pe;
                            } else if (band0->avoid_holes != PSY_3GPP_AH_NONE &&
                                       0.4f * psy_coeffs[g].min_snr * band1->energy > band0->energy) {
                                band0->avoid_holes = PSY_3GPP_AH_NONE;
                                band0->thr = 2.0f * band0->energy;
                                pe -= band0->pe;
                            }
                            if (pe <= desired_pe)
                                goto finish;
...
av_log(NULL, AV_LOG_INFO, "5.6.1.3.7b: pe = %f, desired = %f\n", pe, desired_pe);
                for (ch = 0; ch < group->num_ch; ch++) {
                    AacPsyChannel *pch  = &pctx->ch[channel + ch];
                    ...
                    for (w = 0; w < wi[ch].num_windows*16; w += 16) {
                        for (g = 0; g < num_bands[ch]; g++) {
                            ...
                            if (band->avoid_holes != PSY_3GPP_AH_NONE && band->energy > band->thr) {
                                min_energy  = FFMIN(min_energy, band->energy);
                                avg_energy += band->energy;
                                bands++;
                            }
                        }
                    }
                }
                av_log(NULL, AV_LOG_INFO, "min = %f, avg = %f, bands = %d, ",
                       min_energy, avg_energy, bands);
                assert(bands != 0);
                avg_energy = avg_energy / bands;
                av_log(NULL, AV_LOG_INFO, "avg = %f\n", avg_energy);
                sfb = 0;
                for (i = 0; i < 4; i++) {
                    border = min_energy * powf(avg_energy / min_energy, (2.0f * i + 1.0f) / 7.0f);
                        ...
                        for (ch = 0; ch < group->num_ch; ch++) {
                            ...
                            if (g >= 0) {
                                for (w = 0; w < wi[ch].num_windows*16; w+= 16) {
                                    ...
                                    if (band->avoid_holes != PSY_3GPP_AH_NONE && band->energy < border) {
                                        band->avoid_holes = PSY_3GPP_AH_NONE;
                                        band->thr = 2.0f * band->energy;
                                        pe -= band->pe;
                                    }
                                    if (pe <= desired_pe)
                                        goto finish;
...
av_log(NULL, AV_LOG_INFO, "5.6.1.3.7c: pe = %f, desired = %f, border = %f\n",
...
finish:
av_log(NULL, AV_LOG_INFO, "final: pe = %f, desired = %f\n", pe, desired_pe);
    for (ch = 0; ch < group->num_ch; ch++) {
        AacPsyChannel *pch  = &pctx->ch[channel + ch];
        for (w = 0; w < wi[ch].num_windows*16; w += 16) {
            for (g = 0; g < num_bands[ch]; g++) {
                AacPsyBand *band     = &pch->band[w+g];
                FFPsyBand  *psy_band = &ctx->ch[ch].psy_bands[w+g];
